...
// ---------------------------------------------------------------------------
// getAzureReachability() - "is Azure up?" (control-plane reachability, NOT run status)
// Acquires a token (proves Entra login is up) then makes one lightweight ARM
// call. We only care that Azure responds, not whether any workflow succeeded.
// ---------------------------------------------------------------------------
async function getAzureReachability() {
  ...
  try {
    ...
    return {
      status: 'healthy',
      mode: 'azure',
      detail: `Azure reachable — ${count} subscription${count === 1 ? '' : 's'} visible.`,
      lastError: null,
      checkedAt,
    };
  } catch (err) {
    // Azure answered but rejected us: the platform is up, our access isn't.
    ...
    // No / non-2xx response: treat Azure as unreachable.
    return {
      status: 'down',
      mode: 'azure',
...
module.exports = { getLogicAppHealth, getAllWorkflowHealth, getAzureReachability };
